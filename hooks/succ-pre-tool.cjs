...
const DANGEROUS_PATTERNS = [
  ...
  { pattern: /\brm\s+-r\s+~(?:\s|\/|$)/, reason: 'rm -r ~ would destroy the entire home directory.' },
  ...
  // ── Infrastructure ──
  { pattern: /\bterraform\s+destroy\b/, reason: 'terraform destroy tears down all managed infrastructure.' },
  { pattern: /\bkubectl\s+delete\s+(?:ns|namespace)\b/, reason: 'kubectl delete namespace removes all resources.' },
  { pattern: /\bkubectl\s+delete\s+.*--all\b/, reason: 'kubectl delete --all removes all resources of type.' },
  ...
  // ── MongoDB ──
  { pattern: /\bmongo(?:sh)?\b.*\bdropDatabase\b/, reason: 'dropDatabase deletes a MongoDB database.' },
  ...
  // ── Lockfiles ──
  { pattern: /\brm\s+.*package-lock\.json\b/, reason: 'Deleting package-lock.json causes dep issues.' },
  ...
  // ── Exfiltration ──
  { pattern: /\bcurl\b.*(?:-d\b|--data\b|--form\b)/, reason: 'curl with data flags can exfiltrate information.', exemptLocalhost: true },
  ...
  { pattern: /\bbase64\b.*\|\s*\bcurl\b/, reason: 'Piping base64 to curl is exfiltration pattern.' },
  { pattern: /\bcat\b.*\|\s*\bcurl\b/, reason: 'Piping file to curl can exfiltrate data.' },

  // ── Supply chain ──
  { pattern: /\bcurl\b.*\|\s*(?:bash|sh|zsh)\b/, reason: 'Piping curl to shell runs untrusted code.' },
  { pattern: /\bwget\b.*\|\s*(?:bash|sh|zsh)\b/, reason: 'Piping wget to shell runs untrusted code.' },
  { pattern: /\bpip\s+install\s+-i\s/, reason: 'pip install -i uses custom index (supply chain risk).' },
  { pattern: /\bnpm\s+install\s+--registry\s/, reason: 'npm --registry uses custom registry (supply chain risk).' },
];
...
const TIER1_PATTERNS = [
  ...
  { re: /<(?:span|div|p)\s+[^>]*(?:display\s*:\s*none|visibility\s*:\s*hidden|hidden)[^>]*>/i, desc: 'Hidden HTML element' },
  { re: /<!--\s*(?:AI|AGENT|LLM|ASSISTANT|IGNORE|INSTRUCTION|SYSTEM|OVERRIDE)/i, desc: 'HTML comment targeting AI' },
];
...
const PROTECTED_DELETE_PATTERNS = [
  ...
  { pattern: /package-lock\.json$/i, reason: 'Lockfile ensures reproducible builds — deletion causes dep issues.' },
  { pattern: /yarn\.lock$/i, reason: 'Lockfile ensures reproducible builds — deletion causes dep issues.' },
  { pattern: /pnpm-lock\.yaml$/i, reason: 'Lockfile ensures reproducible builds — deletion causes dep issues.' },
];
  ...
  if (operation === 'read' || operation === 'write') {
    for (let i = 1; i < parts.length; i++) {
      ...
      if (SENSITIVE_EXTENSIONS.includes(ext)) {
        return { reason: `${ext} files contain private keys/certificates — ${operation} blocked.`, mode: 'deny' };
      }
    }
  }
...
function formatFileContext(memories, fileName) {
  const lines = memories.map((m) => `- [${m.type || 'observation'}] ${sanitize(m.content, 200)}`);
...
process.stdin.on('end', async () => {
  try {
    ...
    if (filePath && (toolName === 'Read' || toolName === 'Write' || toolName === 'Edit')) {
      ...
      if (fileGuardResult) {
        const result = fileGuardResult.mode === 'ask'
