 ...
 *
 * Safety:
 *   - Auto-exit after 90 minutes of no activity (MAX_IDLE_MINUTES) if session abandoned/crashed
 *
...
// Default config - must match DEFAULT_IDLE_WATCHER_CONFIG in src/lib/config.ts
...
// Safety limits
const MAX_IDLE_MINUTES = 90;  // Exit if no activity for this long (session likely abandoned)
...
function isActive() {
  ...
  // Safety: exit if no user activity for too long (session likely abandoned/crashed)
  const now = Date.now();
  const lastUserPrompt = readTimestamp(lastUserPromptFile);
  const lastStop = readTimestamp(lastStopFile);
  const lastActivity = Math.max(lastUserPrompt, lastStop);
  if (lastActivity > 0 && (now - lastActivity) > MAX_IDLE_MINUTES * 60 * 1000) {
    return false;
  }
