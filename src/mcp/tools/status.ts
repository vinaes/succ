/**
 * MCP Status tool — succ_status with actions: overview, stats, score
 */
...
export function registerStatusTools(server: McpServer) {
  server.registerTool(
    ...
    {
      description:
        'Get succ status, token statistics, or AI-readiness score.\n\nExamples:\n- Status: succ_status()\n- Token savings: succ_status(action="stats")\n- AI-readiness: succ_status(action="score")',
      inputSchema: {
        action: z
          .enum(['overview', 'stats', 'score'])
          .optional()
          .default('overview')
          .describe(
            'overview = index/memory/daemon status, stats = token savings, score = AI-readiness'
          ),
            ...
            try {
              ...
              if (freshness.stale > 0 || freshness.deleted > 0) {
                status.push(
                  ...
                  freshness.stale > 0
                    ? `  Stale (modified since indexing): ${freshness.stale}`
                    : '',
                  freshness.deleted > 0
        ...
        case 'stats': {
          try {
            ...
            if (!summaryEnabled) {
              ...
              lines.push(
                `  Transcript: ${formatTokens(sessionStats.total_full_source_tokens)} tokens`
              );
            ...
            if (totalSaved > 0) {
              ...
              lines.push(
                `  Queries: ${ragTotalQueries}, ${formatTokens(ragTotalReturned)} returned`
              );
