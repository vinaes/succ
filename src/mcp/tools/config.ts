...
export function registerConfigTools(server: McpServer) {
  ...
  server.tool(
    'succ_config_set',
    'Update succ configuration values. Saves to global (~/.succ/config.json) or project (.succ/config.json). Common keys: embedding_mode (local/openrouter/custom), analyze_mode (claude/local/openrouter), llm.claude_mode (process/ws), openrouter_api_key, embedding_api_url, analyze_api_url, analyze_model, quality_scoring_enabled, sensitive_filter_enabled, graph_auto_link, idle_reflection.enabled, idle_watcher.enabled',
    {
