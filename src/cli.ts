...
program
  ...
  .description(
    'Knowledge graph: export, stats, auto-link, enrich-relations, proximity, communities, centrality, cleanup'
  )
  .option('-f, --format <format>', 'Export format: obsidian or json', 'obsidian')
  .option('-t, --threshold <number>', 'Similarity threshold for auto-link/cleanup prune', '0.75')
  .option('--orphan-threshold <number>', 'Similarity threshold for orphan reconnection', '0.60')
  ...
  .option('--skip-enrich', 'Skip LLM enrichment step (cleanup)')
  .option('--skip-orphans', 'Skip orphan reconnection (cleanup)')
  .option('--skip-finalize', 'Skip communities + centrality rebuild (cleanup)')
  .action(async (action, options) => {
    ...
    return graph({
      ...
      threshold: parseFloat(options.threshold),
      orphanThreshold: options.orphanThreshold ? parseFloat(options.orphanThreshold) : undefined,
