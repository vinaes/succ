...
describe('Sensitive Filter', () => {
  ...
  describe('Phone Numbers Detection', () => {
    it('should detect Russian phone numbers', () => {
      const result = scanSensitive('Call me at 79146724512');
      expect(result.hasSensitive).toBe(true);
      expect(result.matches.some(m => m.type === 'phone_ru' || m.type === 'long_number')).toBe(true);
    });
    ...
    it('should detect Belarus phone numbers', () => {
      const result = scanSensitive('Phone: 375298448197');
      expect(result.hasSensitive).toBe(true);
      expect(result.matches.some(m => m.type === 'phone_by' || m.type === 'long_number')).toBe(true);
    });
  ...
  describe('Redaction', () => {
    it('should properly redact multiple sensitive items', () => {
      const text = 'Key: sk-ant-abc12345678901234567890 Phone: 79146724512';
      const result = scanSensitive(text);
      expect(result.redactedText).not.toContain('sk-ant');
      expect(result.redactedText).not.toContain('79146724512');
    });
